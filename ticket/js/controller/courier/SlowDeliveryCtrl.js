...
//Ticket
angular.module('app').controller('SlowDeliveryCtrl', ['$scope', '$http', '$state', '$rootScope', 'PhpJs', 'Order', 'Config_Status', 'Config', '$modal', 'Location', '$timeout', 'city', 'district',
    function ($scope, $http, $state, $rootScope, PhpJs, Order, Config_Status, Config, $modal, Location, $timeout, city, district) {
        ...
        $scope.loadPage = function ( page, cmd) {
            ...
            var url = ApiPath + 'ticket-dashbroad/slow-delivery?limit=' + $scope.item_page + '&to_city_id=' + $scope.frm.to_city + '&to_district_id=' + $scope.frm.to_district +'&page=' + page + '&city_id=' + $scope.selectedCity + '&district_id=' + $scope.selectedDistrict;
