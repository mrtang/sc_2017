...
angular.module('app.directives', ['ui.load'])
  .directive('status', function(Config_Status) {
    return {
      restrict: 'A',
      link: function(scope, element, attrs) {
        var data_status      = Config_Status.ticket_btn;
        element.addClass(data_status[attrs.status].bg);
        element.html(data_status[attrs.status].name);
      }
    }
  })
...
.directive('caseTypeTicket', function () {
    return {
        restrict: 'A',
        scope: {
            caseId:         '=',
        ...
        link: function ($scope,$element,$attrs) {
            $scope.$watch('caseId', function (new_value, old_value) {
                if(new_value != undefined && new_value > 0 && $scope.listType.length > 0){
                    $scope.ltype = $scope.listType;
        ...
        template: '<option value="">{{defaultLabel}}</option><option value="{{type}}" ng-repeat="type in ltype | filter:{case_id: caseId }">{{type.type_name}}</option>'
    }
})
                    ...
                    Notify.get().then(function (result) {
                        if(!result.data.error){
                            ...
                            angular.forEach(scope.listNotify, function(value, key) {
                                ...
                                switch(value.type) {
                                    case 1:
                                        scope.listNotify[key].href = ApiUrl+"ticket/request/management/90/"+value.refer;
                                        break;
                                    case 2:
                                        scope.listNotify[key].href = SellerUrl+"order/detail/"+value.refer;
                                        break;
                                    default:
                                        scope.listNotify[key].href = ApiUrl+"ticket/request/management/7/";
                                        break;
                                }
