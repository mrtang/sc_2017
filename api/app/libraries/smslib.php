...
class Smslib {
    ...
    public static function send(){
        $LMongo         = new LMongo;
        $start = time() - 2*86400;
        $dbData = $LMongo::collection('log_send_sms')->where('status',0)->whereIn('telco',array('vina','viettel','mobi','vietnam','beeline','sfone'))->whereGt('time_create',$start)->take(10)->get()->toArray();
    ...
    public static function sendacc(){
        $LMongo         = new LMongo;
        $start = time() - 2*86400;
        $dbData = $LMongo::collection('log_send_sms')->where('status',0)->where('priority',1)->whereIn('telco',array('vina','viettel','mobi','vietnam','beeline','sfone'))->whereGt('time_create',$start)->take(1)->get()->toArray();
        // return self::_process_sending_viettel();
        // return $dbData;
        if(!$dbData){
            die('NOT_EXIST_DATA_SMS');
    ...
    private static function _process_sending_viettel_soap($dbData = array()) {
        $LMongo         = new LMongo;
        //
        $soapClient = new SoapClient("https://sms.viettelpost.com.vn/Sms.asmx?wsdl");
        $header[]     = new SoapHeader('http://tempuri.org/', 'ServiceAuthHeader', array('Token' => (string)'9086203E4A9F552A3C30C3748A3D9178'));
        ...
        $result =  $soapClient->InsertSMSMSG($smsParams);
        $arrSms     = array();
        if($result->InsertSMSMSGResult == 1){
            # insert du lieu vao bang log_sms
        ...
        var_dump($result);echo 'vtp<hr><br>';
    }
    ...
    private static function _process_sending_viettel(){
        ...
        SoapWrapper::add(function ($service) {
            $service->name('SoapClientLogin')->wsdl('http://203.113.130.254:8089/UserName.asmx?wsdl')->trace(true);
        });
        ...
        SoapWrapper::service('SoapClientLogin', function ($service){
            ...
            try {
                ...
                SoapWrapper::add(function ($service) use ($headers){
                    $service->name('SoapClient')->wsdl('http://203.113.130.254:8089/Sms.asmx?wsdl')
    ...
    {        
        $arrViettel = array('096','097','098','016','086','089');
        $arrMobi    = array('090','093','0120','0121','0122','0126','0128');
        $arrVina    = array('091','094','0123','0124','0125','0127','0129','088');
        $arrVietnam = array('092','0188','018');
        $arrBeeline = array('0996','0199','099');
        $arrSfone   = array('095');
        ...
        if(in_array($dau4so,$arrBeeline)  || in_array($dau3so,$arrBeeline)){
            return 'beeline';
        }
