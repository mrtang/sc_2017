...
require_once (realpath(dirname(__FILE__) . "/../entity/ZaloMapErrorResult.php"));
...
class ZaloSdkHelper {
    ...
    public static function sendRemoveFan($client) {
        $request = $client->get();
        $response = $request->send()->json();

        $error = $response["error"];
        $result = new ZaloMapErrorResult();
        ...
        if (!empty($response["result"])) {
            $result->setMapError($response["result"]);
        }
    ...
    public static function sendQueryProfile($client) {
        ...
        if ($error == -1) {
            ...
            if (!empty($response["result"])) {
                ...
                $zaloProfile->setPageId($response["result"]["userId"]);
                return $zaloProfile;
            } else {
                $zaloSdkExcep = new ZaloSdkException();
                $zaloSdkExcep->setZaloSdkExceptionErrorCode(-1);
