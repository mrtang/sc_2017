...
use bm_warehousemodel\PutawayItemModel;

class WareHousePutawayCtrl extends BaseCtrl {
    ...
    private function getModel(){
        ...
        $PutAwayStatus  = Input::has('putaway_status')      ? (int)Input::get('putaway_status')             : null;
        $ListStatus     = Input::has('list_status')         ? trim(Input::get('list_status'))               : '';


        $Model          = new PutawayItemModel;
        $ListUser       = [];
        ...
        if(($PutAwayStatus)){
            $Model          = $Model->where('status',$PutAwayStatus);
        }

        if(!empty($TrackingCode)){
                if(preg_match("/^PA/i", $TrackingCode)) {
                    $Model = $Model->where('put_away_code', $TrackingCode);
