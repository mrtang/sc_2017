...
class UploadController extends \BaseController {
	...
	public function postTicket($id)
	{
        ...
        $new_filename = $this->Encrypt32($UserInfo['email'].$UserInfo['id'].$this->time());
        ...
        $InsertAttach   = $AttachModel::insert(array('refer_id' => $id,'type' => $Type, 'name' => $FileName,'link_tmp' => $LinkUpload['linkPath'].'/'.$file->getNameWithExtension(), 'extension' => $file->getExtension(),'time_create' => $this->time()));
    ...
    public function postExchange($id = "")
    {
        ...
        if(empty($id)){
            return Response::json([
                'error'         => true,
                'error_message' => "Lỗi khi tải file vui lòng thử"
            ]);
        }

        $LinkUpload = $this->Createfolder(\Hash::make($this->time()));

        $storage    = new Upload\Storage\FileSystem($LinkUpload['uploadPath']);
        $file       = new Upload\File('ExchangeFile', $storage);
        $FileName   =  $file->getNameWithExtension();
        // Optionally you can rename the file on upload
        $new_filename = $this->Encrypt32($id.$this->time());
        $file->setName($new_filename);
        ...
        try{
            $ExchangeAttachModel::insert(['exchange_id' => $id, 'name' => $FileName,'link_tmp' => $LinkUpload['linkPath'].'/'.$file->getNameWithExtension(), 'extension' => $file->getExtension(),'time_create' => $this->time()]);
            $contents   = array(
                'error'     => false,
                'messege'   => 'SUCCESS'
            );
