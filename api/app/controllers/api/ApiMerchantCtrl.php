...
class ApiMerchantCtrl extends \BaseController {
    ...
    private function _getMerchantInfo(){
        if (Session::has('user_info'))
        {
            if(Session::get('user_info')['parent_id'] > 0){
                ...
                $this->childId  = Session::get('user_inf')['oid'];
            }
            else{
	...
	public function postStatus()
	{
        ...
        $dbOrder = ordermodel\OrdersModel::whereIn('tracking_code',$tracking_code)
                                ...
                                ->where(function($query){
                                    $query->where('time_accept','>=', time() -  30 * 86400)
    ...
    */
    public function postCancel(){
        ...
        /*if(empty($result['data_log'])){
            $result['error']         = true;
            $result['message']       = "";
            $result['error_message'] = "Không có thay đổi";
            goto done;
        }*/
