...
use bm_accmodel\CashOutModel;
use bm_accmodel\MerchantModel;
use bm_accmodel\TransactionModel;
use bm_accmodel\RefundModel;
use bm_accmodel\RecoverModel;

class CashOutCtrl extends BaseCtrl {
    private $master_id      = 1;
    private $table_log      = '';
    ...
    public function postUpload(){
        $this->table_import             = 'log_bm_import_cash_out';
        $this->table_log                = 'log_bm_cash_out';

        $UploadCtrl                     = new \accounting\UploadCtrl;
    ...
    public function postUploadRefund(){
        $this->table_import     = 'log_bm_import_refund';
        $this->table_log        = 'log_bm_refund';
    ...
    private function getUserByEmail($email){
        $UserModel  = new \User;
        $User       = $UserModel::where('email',$email)->first(['id', 'email','organization']);
    ...
    **/
      public function getCashOut($id){
        ...
        if($this->total > 0){
            ...
            $Merchant   = $MerchantModel->firstOrNew(array('merchant_id' => (int)$User->organization));
            ...
            try{
                $TransactionModel::insert([
                    'type'              => 3,
                    'transaction_id'    => !empty($Item['transaction_id']) ? $Item['transaction_id'] : 'SC_Chuyen_Tien',
                    'refer_code'        => !empty($Item['transaction_id']) ? $Item['transaction_id'] : 'SC_Chuyen_Tien',
                    'from_user_id'      => (int)(int)$User->organization,
            ...
            try{
                $CashOutModel::insert([
                    'merchant_id'       => (int)$User->organization,
                    'amount'            => $Item['amount'],
                    'transaction_id'    => !empty($Item['transaction_id']) ? $Item['transaction_id'] : 'SC_Chuyen_Tien',
                    'refer_code'        => !empty($Item['transaction_id']) ? $Item['transaction_id'] : 'SC_Chuyen_Tien',
                    'time_create'       => $this->time(),
            ...
            try{
                MerchantModel::where('merchant_id', (int)$this->master_id)->increment('balance', $Item['amount']);
            }catch(Exception $e){
    ...
    public function getRefund($id){
        ...
        if($this->total > 0){
            ...
            try{
                $TransactionModel::insert([
                    'type'              => 4,
                    'transaction_id'    => !empty($Item['transaction_id']) ? $Item['transaction_id'] : 'SC_Hoan_Tien',
                    'refer_code'        => !empty($Item['transaction_id']) ? $Item['transaction_id'] : 'SC_Hoan_Tien',
                    'from_user_id'      => (int)$this->master_id,
                    'to_user_id'        => (int)(int)$User->organization,
            ...
            // Update Balance Master
            try{
                MerchantModel::where('merchant_id', (int)$this->master_id)->decrement('balance', $Item['amount']);
            }catch(Exception $e){
                $this->code             = 'UPDATE_BALANCE_MASTER_FAIL';
                $this->message_error    = 'Cập nhật số dư hệ thống thất bại';
                $this->data_update = [
    ...
    public function getRecover($id){
        $this->table_import     = 'log_bm_import_recover';
        $this->table_log        = 'log_bm_recover';
        ...
        if($this->total > 0){
            ...
            try{
                $TransactionModel::insert([
                    'type'              => 3,
                    'transaction_id'    => !empty($Item['transaction_id']) ? $Item['transaction_id'] : 'SC_Thu_Hoi',
    ...
    function getListExcel($id){
        ...
        if($type == 'refund'){
            $ListModel      = LMongo::collection('log_bm_refund')->where('partner', $id);
        }elseif($type == 'recover'){
            $ListModel      = LMongo::collection('log_bm_recover')->where('partner', $id);
        }else{
            $ListModel      = LMongo::collection('log_bm_cash_out')->where('partner', $id);
        }
