...
class OrderESController extends \BaseController {
    ...
    private function GetModelES(){
        ...
        $CheckVerify        = Input::has('check_verify')        ? (int)Input::get('check_verify')               : 0;
        ...
        if($TimeStart > 0){
            $TimeCreateStart    = $this->time() - $TimeStart*86400;
        }
    ...
    private function GetModel(){
        ...
        $FromCountryId      = Input::has('from_country_id')     ? (int)Input::get('from_country_id')            : 0;
        $CreateBy           = Input::has('createby')            ? (int)Input::get('createby')                   : 0;
        ...
        if(!empty($Search)){
            ...
            $PhoneCleared = preg_replace('/[() .+-]/', '', $Search);

            if (filter_var($Search, FILTER_VALIDATE_EMAIL)){  // search email
                $Model          = $Model->where('to_email',$Search);
            }elseif(filter_var((int)$Search, FILTER_VALIDATE_INT,array('option'=>array('min_range'=>8,'max_range'=>20))) || (count($PhoneCleared)>10)) {  // search phone
        ...
        if($CreateBy > 0){
            $Model          = $Model->where('createby', $CreateBy);
        }
        ...
        if(!empty($CheckVerify)){
            if($CheckVerify == 1){
               ...
               $Model = $Model->where('verify_id',  0)->whereIn('status', [52,53,66,67]);
            }
        }
    ...
    public function getOrder(){ // seller center - management
        ...
        if(!empty($Status) && $Status == 'delivering'){
            $Model = $Model->where('time_success', 0)->whereNotIn('status', [52,53,66,67]);
        }

        if($Cmd == 'export'){
            return $this->ExportExcel($Model->with(['OrderDetail', 'OrderItem', 'OrderItems','OrderFulfillment'])->get(
                array(
                        'orders.id',
                        'tracking_code',
                        'status',
    ...
    */
    ...
    public function getGroupOrderProcess (){
        ...
        $ModelOverWeight    = clone $Model;
        $ListOverWeightId   = $ModelOverWeight->where('accept_overweight', 0)->where('verify_id', 0)->where('time_update', 'gte', $this->time() - 172800)->lists('id');
        $totalOverWeight    = 0;
        ...
        $DataGroup       = $Model->get(array('id', 'status'))->toArray();
    ...
    */
    ...
    public function getOrderfee(){
        ...
        if($Cmd == 'export'){
            ...
            return $this->ExportExcel($Model->with(['OrderDetail', 'OrderItems'])->get()->ToArray());
        }
    ...
    public static function getShow($code)
    {
        ...
        ))->first(array('id','tracking_code','service_id','courier_id','to_name','to_phone','to_email','status','from_address_id','to_address_id','time_create','time_accept', 'warehouse'));
        ...
        if (Input::hasFile('file')) {
            ...
            if(in_array((string)$extension, array('csv','xls','xlsx')) && in_array((string)$MimeType,array('text/plain','application/vnd.ms-excel','application/vnd.ms-office','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'))){
                ...
                if(!empty($id)){
                 ...
                 if($ReadExcel){
                     return $this->_ResponseData([], ['id'=> $id]);
                 }else{
                     return $this->_ResponseData([]);
                 }
                 
                }else{
                     $this->_message = "Tải lên không thành công";
                     return $this->_ResponseData([]);
                    
                     
                }  
            }else{
                $this->_message = "File không đúng định dạng";
                return $this->_ResponseData([]);
    ...
    public function getSynonyms($id){
        $LMongo     = new LMongo;
        $Lading     = $LMongo::collection('log_list_lading')->find($id);
        $Update     = [];
        ...
        if(empty($Lading['to_city'])){
            /*
            $Model  = new \SynonymsModel;
            $Model  = $Model->whereNull('district_id');

            $query  = 'MATCH (synonyms) AGAINST (\''.$Lading['city_name'].'\')';

            $Value  = $Model->whereRaw($query)->orderBy('score','DESC')
                ->select(['city_id','district_id',DB::raw($query.' AS score')])
                ->get()->toArray();
            */
            $ElasticSearchController    = new \ElasticSearchController;
            $Value = $ElasticSearchController->getLocation($Lading['city_name'],0);
            ...
            $Update['to_city']  = (int)$Value[0]['_source']['city_id'];
        }

        $CityId = isset($Update['to_city']) ? $Update['to_city'] : $Lading['to_city'];

        if(empty($Lading['to_district'])){
            /*
            $Model  = new \SynonymsModel;
            ...
            $query  = 'MATCH (synonyms) AGAINST (\''.$Lading['district_name'].'\')';
            ...
            */
            ...
            $ElasticSearchController    = new \ElasticSearchController;
            $Value = $ElasticSearchController->getLocation($Lading['district_name'], $CityId);
            if(isset($Value[0]) && !empty($Value[0])){
                $Update['to_district']  = (int)$Value[0]['_source']['district_id'];
            }
        }

        $DistrictId = isset($Update['to_district']) ? $Update['to_district'] : $Lading['to_district'];

        if(!empty($Update)){
            $LMongo     = new LMongo;
            $LMongo     = $LMongo::collection('log_list_lading');
            $Update     = $LMongo->where('_id', new \MongoId($id))
                ->where('active',0)
                ->update($Update);
    ...
    // Kiểm tra các cột tạo đơn excel
    private function CheckFieldExcel($FieldColumn){
        $result = [];
        $Field  = [
            'DICH_VU'               => 'service',
            'HINH_THUC_THANH_TOAN'  => 'vas',
            'DICH_VU_DACBIET'       => 'vas',
            'NOI_DUNG_HANG_HOA'     => 'item_name',
            'HANG_HOA_HOAC_SKU'     => 'item_name_sku',
            'KHO_HANG_BOXME'        => 'boxme_inventory',
            'TRONG_LUONG_GRAM'      => 'item_weight',
            'SO_LUONG'              => 'item_qty',
            'TONG_TIEN_HANG'        => 'item_price',
            'GHI_CHU_DON_HANG'      => 'item_desc',
            'TEN_NGUOI_NHAN'        => 'to_name',
            'DIEN_THOAI'            => 'to_phone',
            'TINH_THANH'            => 'to_city',
            'QUAN_HUYEN'            => 'to_district',
            'SO_NHA_TEN_DUONG'      => 'to_address',
            'TONG_TIEN_THU_HO'      => 'collect'
        ];
    ...
    function ReadExcel($id){
        ...
        if($Data){
            $DataInsert = array();
            $Field  = $this->CheckFieldExcel($Data[0]);
            if(!$Field){
                $this->_message = "Thiếu cột trong file excel, vui lòng thử lại";
                $this->_error   = true;
                return false;
            }
            ...
            foreach($Data as $key => $val){
                if($key > 0){
                    if(!empty($val[$Field['vas']]) &&  !empty($val[$Field['to_name']]) &&
                        !empty($val[$Field['to_phone']]) && !empty($val[$Field['to_city']]) && !empty($val[$Field['to_district']]) &&
                        !empty($val[$Field['to_address']])){

                        //!empty($val[$Field['item_weight']]) &&
                        //!empty($val[$Field['item_price']]) && 

                        $val[$Field['collect']] = isset($val[$Field['collect']]) ? $val[$Field['collect']] : 0;
                        $phone = preg_replace("/[^0-9.,]/", "", $val[$Field['to_phone']]);
                        ...
                        $list_quantity_arr  = [];
                        $order_items        = [];
                        $isBoxmeInvetory    = false;
                        if(isset($Field['boxme_inventory']) && explode('-', $val[$Field['boxme_inventory']])[0] == 2){
                            $isBoxmeInvetory = true;
                        }
                        
                        if($isBoxmeInvetory){
                            $list_sku_arr       = explode(',', trim($val[$Field['item_name_sku']]));
                            $list_quantity_arr  = explode(',', $val[$Field['item_qty']]);

                            if(count($list_sku_arr) != count($list_quantity_arr)){
                                $this->_message = "Số lượng SKU, không khớp với số lượng ";
                                $this->_error   = true;
                                $this->_additional['sku'] = $list_sku_arr;
                                $this->_additional['quantity'] = $list_quantity_arr;
                                
                                return false;
                            } 

                            foreach($list_sku_arr as $key => $value){
                                $order_items[] = [
                                    'sku' => trim($value),
                                    'quantity'=> isset($list_quantity_arr[$key]) ? trim($list_quantity_arr[$key]) : 1
                                ];
                            }
                        }
                        ...
                        // city
                        $arr_city       = explode('_',$val[$Field['to_city']]);
                        if(count($arr_city) >= 2 && ((int)end($arr_city) > 1 || ($val[$Field['to_city']] == 'Cao_Bằng_1'))){
                            $city_id        = (int)end($arr_city);
                            array_pop($arr_city);
                            $city_name      = implode(' ',$arr_city);
                        }else{
                            $city_id    = 0;
                            $city_name  = $val[$Field['to_city']];
                        }


                        // district
                        $arr_district       = explode('-',$val[$Field['to_district']]);
                        if(count($arr_district) >= 2 && ((int)(end($arr_district) > 1 || ($val[$Field['to_district']] == 'Huyện Thạch An-1')))){
                            $district_id        = (int)end($arr_district);
                            array_pop($arr_city);
                            $district_name      = implode(' ',$arr_district);
                        }else{
                            $district_id    = 0;
                            $district_name  = $val[$Field['to_district']];
                        }
                        ...
                        try{
                            ...
                            $DataInsert[] = array(
                                ...
                                'status'            => 'NOT ACTIVE',
                                'active'            => 0,
                                'service'           => (int)$val[$Field['service']] > 0 ? (int)$val[$Field['service']] : 2,
                                'vas'               => (int)$val[$Field['vas']],
                                'item_name'         => isset($Field['item_name']) ?  trim($val[$Field['item_name']]) : (isset($Field['item_name_sku']) ?  trim($val[$Field['item_name_sku']]) : ""),
                                'item_sku'          => isset($Field['item_name_sku']) ?  trim($val[$Field['item_name_sku']]) : "",
                                'order_items'       => $order_items,
                                'item_weight'       => (int)preg_replace("/[^0-9]/", "", $val[$Field['item_weight']]),
                                'item_qty'          => isset($Field['item_qty']) ? $val[$Field['item_qty']]  : 1,
                                
                                //'item_qty'          => isset($Field['item_qty']) ((int)preg_replace("/[^0-9]/", "", $val[$Field['item_qty']])) > 0 ? (int)preg_replace("/[^0-9]/", "", $val[$Field['item_qty']]) : 1,
                                'item_price'        => preg_replace("/[^0-9]/", "", $val[$Field['item_price']]),
                                'item_desc'         => isset($Field['item_desc']) && isset($val[$Field['item_desc']]) ? trim($val[$Field['item_desc']]) : "",
                                'to_name'           => trim($val[$Field['to_name']]),
                                'boxme_inventory'   => $isBoxmeInvetory,
                                'to_phone'          => $phone,
                                'checking'          => 1,
                                'to_city'           => $city_id,
                                'to_district'       => $district_id,
                                'to_address'        => trim($val[$Field['to_address']]),
                                'collect'           => preg_replace("/[^0-9]/", "", $val[$Field['collect']]),
                                'city_name'         => $city_name,
    ...
    function postChangelog($id){
        ...
        * */
        ...
        if(isset($Data['item_name'])){
            ...
            if($ItemModel['boxme_inventory'] == true){
                $list_sku_arr       = explode(',', $Data['item_qty']);
                $list_quantity_arr  = explode(',', $Data['item_name']);

                if(count($list_sku_arr) != count($list_quantity_arr)){
                    $contents = array(
                        'error'     => true,
                        'message'   => 'Số lượng không tương ứng với số SKU',
                        'data'      => $id
                    );
                    return Response::json($contents);
                } 
                ...
                $DataUpdate['order_items'] = $order_items;
        ...
        if(isset($Data['item_qty'])){
            ...
            if($ItemModel['boxme_inventory'] == true){
                $DataUpdate['item_qty']         = $Data['item_qty'];
        ...
        if(!empty($DataUpdate)){
            ...
            if($Update){
                $contents = array(
                    ...
                    'item_name'     => count(explode(',', $Data['item_name'])),
                    'item_quantity' => count(explode(',', $Data['item_qty'])),
                    'boxme'         => $ItemModel['boxme_inventory'] == true,
                    'boxme_v'       => $ItemModel['boxme_inventory']
    ...
    public function getCreateorder($id){
        ...
        $Accept         = Input::has('accept')      ? (int)Input::get('accept')     : 0;
        ...
        if(isset($Order['boxme_inventory']) && $Order['boxme_inventory'] == true){
            $DataCreate['Domain'] = 'boxme.vn';
            if(empty($Stock['warehouse_code'])){
                 $contents = array(
                    'error'     => true,
                    'message'   => 'Kho bạn chọn, không phải kho Boxme',
                    'data'      => "",
                    'code'      => 1
                );
                return Response::json($contents);
            ...
            if(strlen(strstr($Stock['warehouse_code'], 'VTP')) > 0 && $Stock['warehouse_code'] !== 'VTPHN02'){
                $DataCreate['Courier'] = 1;
            }
            
            $ListSku = explode(',', $Order['item_name']);
            $ListProduct = \fulfillmentmodel\SellerProductModel::whereIn('sku', $ListSku)->where('user_id', $UserInfo['id'])->get()->toArray();
            if(count($ListProduct) !== count($Order['order_items'])){
                $contents = array(
                    'error'     => true,
                    'message'   => 'Item không tồn tại trên hệ thống',
                    'data'      => [
                        'list_sku'  => $ListSku,
                        'list_prod' => $ListProduct,
            ...
            foreach($Order['order_items'] as $value){
                foreach($ListProduct as $product){
                    if($value['sku'] == $product['sku']){
                        $Items[] = [
                            'Name'      => $product['name'],
                            'Price'     => $product['price_sale'] * $value['quantity'],
                            'Quantity'  => $value['quantity'],
                            'Weight'    => $product['weight'] * $value['quantity'],
                            'BSIN'      => $product['sku']
                        ];
                        $OrderQuantity += $value['quantity'];
                        $OrderWeight   += $product['weight'] * $value['quantity'];
                        $OrderPrice    += $product['price_sale'] *  $value['quantity'];
                        $OrderName[]    = $product['name'];
                    }  
                }
            }

            $DataCreate['Items']             = $Items;
            $DataCreate['Order']['Weight']   = $OrderWeight;
            $DataCreate['Order']['Amount']   = $OrderPrice;
            $DataCreate['Order']['ProductName']     = implode(', ', $OrderName);
            $DataCreate['Order']['Quantity'] = $OrderQuantity;
        }
    ...
    private function isReturn($status){
        return in_array($status, [66, 67]);
    }
            ...
            $excel->sheet('Sheet1', function($sheet) use($Data, $Courier, $Service, $City, $Address, $District, $Ward, $User, $Status, $FromAddress){
                ...
                foreach ($Data as $val) {
                    ...
                    $dataExport = array(
                        ...
                        (isset($val['order_detail']) && !$this->isReturn($val['status'])) ? number_format($val['order_detail']['sc_cod']) : '',
                        (isset($val['order_detail']) && !$this->isReturn($val['status'])) ? number_format($val['order_detail']['sc_pbh']) : '',
                        isset($val['order_detail']) ? number_format($val['order_detail']['sc_pvk']) : '',
                        (isset($val['order_detail']) && (!$this->isReturn($val['status']))) ? number_format($val['order_detail']['sc_pch']) : '',
                        isset($val['order_detail']) ? number_format(($val['order_detail']['sc_discount_pvc'] + ((!$this->isReturn($val['status'])) ? $val['order_detail']['sc_discount_cod'] : 0))) : '',
                        (isset($val['order_detail']) && !$this->isReturn($val['status'])) ? number_format($val['order_detail']['money_collect']) : '',
        ...
        return Excel::create($FileName, function($excel) use($Data, $Courier, $Service, $City, $Address, $District, $Ward, $User, $Status){
            $excel->sheet('Sheet1', function($sheet) use($Data, $Courier, $Service, $City, $Address, $District, $Ward, $User, $Status){
                ...
                $sheet->setWidth(array(
                    ...
                    'V'     =>  30, 'W'     =>  30, 'X'     =>  30, 'Y'     =>  30, 'Z'     =>  30, 'AA'     =>  30, 'AB'   =>  30,
                    'AC'     =>  30, 'AD'     =>  30, 'AE'     =>  30, 'AF'  => 30, 'AG'    => 30,   'AH'    =>  30, 'AI'   => 30,
                    'AJ'     => 30, 'AK'    => 30, 'AM'    => 30, 'AN'    => 30, 'AO'    => 30, 'AP' => 30, 'AQ' => 30, 'AR' => 30
                ));
                $sheet->setMergeColumn(array(
                    'columns' => array('A','B','C','D','E','F','G','H','I','J','K' ,'AP','AQ', 'AR'),
                    'rows' => array(
                        array(3,4)
                    )
                ));
                ...
                $sheet->row(4, array(
                    '', '', '', '', '', '', '', '', '', '', '',  'Họ tên', 'Email', 'Số điện thoại',  'Tỉnh/Thành phố',
                    'Quận/Huyện', 'Phường xã', 'Địa chỉ','Họ tên', 'Email', 'Số điện thoại',    'Tỉnh/Thành phố', 'Quận/Huyện', 'Phường xã','Địa chỉ',
                    'Tên sản phẩm', 'Tổng giá trị', 'Khối lượng','Số lượng', 'Mô tả', 'Phí vận chuyển', 'Phí thu hộ', 'Phí bảo hiểm', 'Phí vượt cân', 'Phí chuyển hoàn', 'Phí lưu kho', 'Phí đóng gói', 'Phí xử lý', 'Giảm phí lưu kho', 'Giảm phí đóng gói', 'Giảm phí xử lý'
                ));
                ...
                foreach ($Data as $val) {
                    ...
                    $dataExport = array(
                        ...
                        isset($val['total_amount']) ? $val['total_amount'] : '',
                        isset($val['total_weight']) ? $val['total_weight'] : '',
                        $TotalItem > 0 ? $TotalItem : $val['total_quantity'], 
                        isset($val['order_item']) ? $val['order_item']['description'] : '',

                        isset($val['order_detail']) ? number_format($val['order_detail']['sc_pvc']) : '',
                        (isset($val['order_detail']) && !in_array($val['status'] , [66, 67, 22, 23, 24, 25,27, 28, 29, 78, 121 ])) ? number_format($val['order_detail']['sc_cod']) : '',
                        (isset($val['order_detail']) && !in_array($val['status'] , [66,67,22, 23, 24, 25,27, 28, 29, 78, 121 ])) ? number_format($val['order_detail']['sc_pbh']) : '',
                        isset($val['order_detail']) ? number_format($val['order_detail']['sc_pvk']) : '',
                        (isset($val['order_detail']) && in_array($val['status'] , [66,67,22, 23, 24, 25,27, 28, 29, 78, 121 ])) ? number_format($val['order_detail']['sc_pch']) : '',
                        ...
                        isset($val['order_detail']) ? number_format(($val['order_detail']['sc_discount_pvc'] + ((!in_array($val['status'] , [66, 67, 22, 23, 24, 25,27, 28, 29, 78, 121 ])) ? $val['order_detail']['sc_discount_cod'] : 0))) : '',
                        (isset($val['order_detail']) && !in_array($val['status'] , [66, 67, 22, 23, 24, 25,27, 28, 29, 78, 121 ])) ? number_format($val['order_detail']['money_collect']) : '',
                        ($Payment == 1 ) ? 'Vimo' : 'Ngân Lượng'
                    );
    ...
    public function getDashboard() {
        ...
        $timeStart30Days = $this->time()  - 30*86400;
        ...
        $orderPickupNeedProcess = OrdersModel::where('time_accept','<',$timeToday)
            ...
            ->where(function($query) use($timeYesterday, $timeEndYesterday, $timeToday, $timeStartToday, $currentTime, $timeOff) {
                ...
                $query->orWhere(function($query) use ($timeEndYesterday, $timeStartToday) {
                    ...
                    $query->whereRaw($this->time().' >= '.($timeStartToday+4*3600));
                });
    ...
    public function getReportPickup(){
        $TimeStart          = Input::has('time_start')          ? (int)Input::get('time_start')                 : 0;
        $UserInfo   = $this->UserInfo();
        ...
        $Model  = $Model::where('from_user_id', $UserInfo['id'])->where('time_accept','>=', $this->time() - 86400*$TimeStart);
        ...
        $Data = ['all' => 0];
        if(!empty($ListStatus)){
            $ListGroup = $Model->whereIn('status', $ListStatus)->groupBy('domain')->get(array('domain',DB::raw('count(*) as count')));
            if(!empty($ListGroup)){
                foreach($ListGroup as $val){
                    if(!isset($Data[$val['domain']])){
                        $Data[$val['domain']]   = 0;
                    }
                    $Data[$val['domain']]   = $val['count'];
                    $Data['all']           += $val['count'];
                }
            }
        }
