...
class OrderController extends \BaseController
{
    ...
    public function getIndex() // seller center 7day - 3 month
    {
        ...
        $Data = $Model->with(array(
            ...
            'from_district_id', 'from_ward_id', 'from_address', 'order_code', 'domain', 'to_country_id'))->toArray();
        ...
        if(!empty($ListGroup)) {
            foreach($ListGroup as $val){
                foreach($val['group_order_status'] as $v) {
                    ...
                    $ListStatus[(int)$v['order_status_code']]       = $val['name'] ;
        ...
        if (!empty($Data)) {
            foreach ($Data as $key => $val) {
                ...
                if(isset($ListStatus[$val['meta_status']['code']])){
                    $Data[$key]['meta_status']['group_name'] = $ListStatus[$val['meta_status']['code']];
                }
    ...
    public function getShow($code)
    {
        ...
        $Data = $Model->with(array(
            ...
            'total_quantity', 'to_name', 'to_phone', 'to_email', 'product_name',
            'to_address_id', 'time_accept', 'time_create', 'time_pickup', 'time_update', 'estimate_delivery', 'verify_id', 'from_user_id', 'from_address_id', 'from_city_id',
            'from_district_id', 'from_ward_id', 'from_address', 'order_code', 'to_country_id', 'warehouse'))->toArray();
