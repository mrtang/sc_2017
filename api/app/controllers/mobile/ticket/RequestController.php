...
class RequestController extends \BaseController
{
	...
	public function postCreate()
	{
		...
		if(!empty($Refer)){
			$DataInsert = [];
			if (gettype($Refer) == 'string') {
				$Refer = explode(',', $Refer);
			}
			foreach($Refer as $val){
					$type = 2;
					$val   = trim(strtoupper($val));
					if(preg_match('/^SC\d+$/i',$val)){
						$type = 1;
					}
	...
	public function getShow($id){
		...
		$TimeNow    = time();
		$Model      = new RequestModel;
		$Data       = $Model::where('id','=',$id)->where('user_id', $UserInfo['id'])->with(array('refer','feedback' => function($query) use($UserInfo){
		...
		if (!empty($Data) && !empty($Data['feedback'])) {
			...
			if (empty($Data['rating']) && ($Data['status'] == 'CLOSED' ||$Data['status'] == 'PROCESSED') ) {
				...
				$Data['can_rating'] = false;
			}

			$Data['status_name'] = $this->list_status[$Data['status']];
			...
			if (!empty($listUserId)) {
				...
				foreach ($Data['feedback'] as $key => $value) {
					if (!empty($ListUser[$value['user_id']])) {
						...
						$FeedBackATT = $Data['feedback'][$key]['attach'];
						foreach ($FeedBackATT as $k => $v) {
							$Data['feedback'][$key]['attach'][$k]['link_tmp'] = 'http://cloud.shipchung.vn/'.$v['link_tmp'];
						}
	...
	public function postCloseTicket($id){
		...
		if($Update){
			...
			$contents = array(
				'error'     => false,
				'message'   => 'success',
				'status_name'=> $this->list_status[$Data->status],
