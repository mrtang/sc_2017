...
angular.module('app').controller('CreateShipmentController', [
		...
		$scope.gonext = function() {
			$scope.step2.saveShipment().then(function (shipmentId) {
				...
				// Send a "Add Products Into Warehouse" event to Mixpanel 
				// with a property "Accout Add Products Into Warehouse" 
				...
				mixpanel.track( 
						"ShipmentV2", 
						{"Shipment code":$scope.step3.shipment.RequestCode ? $scope.step3.shipment.RequestCode :"",
		...
		$scope.step4 = function () {
			...
			ShipmentsV2.update_status_shipment($scope.step4.shipment).then(function () {
				...
				mixpanel.track( 
					"ShipmentV2", 
					{"Shipment code":$scope.step4.shipment.RequestCode ? $scope.step4.shipment.RequestCode : "",
					"Email ": $localStorage.email ? $localStorage.email : "",
					"Organization":	$localStorage.organization ? $localStorage.organization: "",
					"Status":"Update status -> Ready to ship"}
				); 
			});
