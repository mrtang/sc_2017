...
angular.module('app').controller('ProblemCtrl', 
    ...
    function ($scope, $modal,   $http,  $rootScope,  toaster,  $filter,  Api_Path,  Order,  PhpJs,  Config_Status) {
        ...
        $scope.get_detail   = function(id){
            ...
            if($scope.detail != null){
                ...
                if($scope.detail.type == 1){
                    ...
                    + '<a class="text-info" target="_blank" href="http://dev.shipchung.vn/#/detail/' + $scope.detail.tracking_code + '">' + $scope.detail.tracking_code +  '</a> - <strong>'
                    ...
                    var note_en = 'Order <a class="text-info" target="_blank" href="http://dev.shipchung.vn/#/detail/' + $scope.detail.tracking_code + '">' + $scope.detail.tracking_code + '</a> change to return status because cannot deliver to receiver(buyer), please contact to them and give us a solution.'
                    var note_vi  = 'Đơn <a class="text-info" target="_blank" href="http://dev.shipchung.vn/#/detail/' + $scope.detail.tracking_code + '">' + $scope.detail.tracking_code + '</a> đã chuyển qua trạng thái chờ xác nhận chuyển hoàn'
                    ...
                    // Lấy thất bại
                    var reason = $scope.detail.reason.split(' - ');
                    var note_en =  'Dear <strong>' + $rootScope.userInfo.fullname + '</strong>, We are sorry for failed picking-up order <a class="text-info" target="_blank" href="http://dev.shipchung.vn/#/detail/' + $scope.detail.tracking_code + '">' + $scope.detail.tracking_code + '</a>'
                    ...
                    var note_vi = 'Gửi <strong>' + $rootScope.userInfo.fullname + '</strong>, ShipChung xin lỗi quý khách về đơn hàng '
                        + '<a class="text-info" target="_blank" href="http://dev.shipchung.vn/#/detail/' + $scope.detail.tracking_code + '">' + $scope.detail.tracking_code + '</a> - ' + $scope.detail.order.product_name + ' chưa thu được do "'
                	...
                	var note_vi = 'Gửi <strong>' + $rootScope.userInfo.fullname + '</strong>, ShipChung xin thông báo quý khách về đơn hàng '
                        + '<a class="text-info" target="_blank" href="http://dev.shipchung.vn/#/detail/' + $scope.detail.tracking_code + '">' + $scope.detail.tracking_code + '</a> - ' + $scope.detail.order.product_name
        ...
        $scope.action_confirm = function(send_zalo, note){
            ...
            bootbox.confirm( msg , function (result) {
                                if(result){
                                    ...
                                    $scope.busy = true;
                                    $scope.change_process(1, 1, 29, 707, note , send_zalo,  function (err, resp ){
                                        $scope.busy = false;
                                        if(!err){
