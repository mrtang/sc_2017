...
.service('Order', ['$http', '$q','$window','Api_Path', 'Storage','toaster','PhpJs','$localStorage','Analytics','$filter', '$rootScope', 
	...
	return {
            ...
            CalculateGlobal : function (data){
                ...
                if(data['Config']['Service'] == 8 || data['Config']['Service'] == 9){
                    urlCalculate = ApiPath+'global/calculate';
                }
                return $http({
                    url: urlCalculate,
                    method: "POST",
                ...
                return $http({
                    ...
                    if(result.error){
                        ...
                        if (data['Config']['AutoAccept'] == 1) {
                            ...
                            mixpanel.track(
                                "Accept Order", 
                                {"Account Accept Order ": "Account Accepted Order"} 
                            ); 

                            mixpanel.people.track_charge(2);
                        }
                ...
                return $http({
                    ...
                    if(!result.error){
                        if(data.status == 21){
                            ...
                            //mixpanel.track("Accept Order item", {Price: 2, "Item Category": "Accept Order"});
                            mixpanel.people.track_charge(2);
                        ...
                        if(data.status == 22 || data.status == 28){
                            ...
                            mixpanel.track( 
                                "Cancel Order", 
                                {" Account Cancel Order ": "Account Cancel Complete"} 
                            ); 
                        }
                        toaster.pop('success', 'Thông báo', 'Cập nhật thành công!');
                    }else{
            ...
            CreateExcel : function(id,stock_id, checking, post_office, accept){
