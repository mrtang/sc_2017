    ...
    .controller('AppCtrl', ['$scope','User', 'Datetime','$rootScope','Site_Config','$location', '$translate', '$localStorage', '$window', '$stateParams', '$state', '$filter','$http', '$modal', '$timeout', 'loginService', 'Notify', 'Base', 'Order', 'toaster','AppStorage', 'Analytics',
            ...
            //$scope.isShipchung = ($location.host() == Site_Config.domain) ? 1: 1;
            	...
            	var Site_BM_Asia =  {
                		...
                		login :{
                			title  		: 'Cổng hậu cần kho vận cho TMDT',
                			title_en   	: 'Cổng hậu cần kho vận cho TMDT',
                			logo 		:'img/BM/logo-BM-Asia.png'
                		},
                		logo_heard		: 'img/BM/logo-BM-Asia.png',
                		titleFooter		:'2016 BOXME - Kho hàng toàn quốc của bạn',
                		titleFooter_en	:'2016 BOXME - Kho hàng toàn quốc của bạn',
                		name   			: 'ShipChung',
                		icon   		 	: 'img/BM/icon.png'
            		}
            //set config site)
...
*/
    ...
    .controller('SigninFormController', 
    		    ['$scope', '$http', '$state', 'loginService', '$localStorage','$filter', '$location', '$facebook', 'sessionService', 'Analytics',
        function ($scope,  $http,  $state,     loginService,   $localStorage,  $filter,   $location,   $facebook,   sessionService,   Analytics) {
    		var tran = $filter('translate');
    		$scope.user             = {};
    ...
    .controller('SignupFbController', ['$scope','loginService', '$q', '$state', 'sessionService', '$facebook', '$http', 'PhpJs', 'Analytics','$filter', 
        ...
        var check = function (resp){
            $http({
                ...
                if(result.error){
                    alert(PhpJs.convertString(result.message));
...
//Modal
   ...
   .controller('ModalSecurity', ['$scope', '$modalInstance', '$http', '$rootScope', 'toaster', 'bootbox', 'UserInfo', 'data', 'type','$filter',
                         function($scope,  $modalInstance,  $http,   $rootScope,    toaster,    bootbox,   UserInfo,   data,   type,  $filter) {
	   var tran = $filter('translate');
