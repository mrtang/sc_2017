    ...
    function($location,   $anchorScroll,  $http,  $timeout,  $filter,  toaster) {
        ...
        return {
            ...
            link: function(scope, el, attr, model) {
                ...
                var url = ApiUrl + 'api/base/list-address';
            ...
            template: '<ui-select ng-model="$parent.selected_location"  ng-disabled="disabled" >'+
                            '<ui-select-match placeholder="{{'+"'ACC_TinhHuyen'"+' | translate}}">{{$select.selected.full_address}}</ui-select-match>'+
                            '<ui-select-choices refresh="CheckCity($select.search)" refresh-delay="400" repeat="value in list_address  | filter: search_str | limitTo: limitcitysearch track by $index">'+
    ...
    .directive('itemAutocomplete', ['$location', '$anchorScroll', '$http', function($location, $anchorScroll, $http) {
        return {
            ...
            link: function(scope, el, attr, ctr) {
                ...
                scope.$watch('productSelected', function (newVal){
                    if (angular.isObject(newVal)) {
                        scope.nameModel   = newVal.product_name;
                    }else {
            ...
            template: '<script type="text/ng-template" id="customTemplate.html">' +
                                      ...
                                      '</a>' +
                        '</script>' +
                        '<input type="text" ng-model="productSelected" name="product_suggestion" placeholder="{{'+"'OBG_Ex_product_name'"+' | translate}}" typeahead-on-select="onsuggestProductSelected($item, $model, $label)" typeahead="item as item.product_name for item in list_items | filter:$viewValue" typeahead-template-url="customTemplate.html" class="form-control">'
        };
    }])
