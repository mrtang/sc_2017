...
angular.module('app').controller('WaitingCtrl', ['$scope', '$rootScope', '$filter', 'Order', 'Config_Status', 'Base',
 	function($scope, $rootScope, $filter, Order, Config_Status, Base) {
        ...
        $scope.frm                  = {group : 31, global: 0, list_status : ''};
        ...
        if($scope.group_order_status[31] == undefined){
            $scope.group_order_status[31]   = Config_Status.group_status[31];
        }
        ...
        Base.PipeStatus(31, 1).then(function (result) {
        ...
        $scope.refresh = function(cmd){
            if($scope.frm.list_status == undefined || $scope.frm.list_status.length  == 0){
                $scope.frm.list_status  = $scope.group_order_status[31].toString();
            }
        ...
        $scope.exportExcel = function(){
            ...
            var html =
                ...
                "<td style='border-style:none'></td>"+
                "<td colspan='3' style='font-size: 18px; border-style:none '><strong>Danh sách chờ xác nhận hoàn</strong></td></tr>" +
                "<tr></tr>" +
            ...
            Order.GetOrder($scope.currentPage,data,'export').then(function (result) {
                if(!result.data.error){
                    ...
                    saveAs(blob, "Danh_sach_cho_xac_nhan_hoan.xls");
                }
            }).finally(function() {
