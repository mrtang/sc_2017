...
angular.module('app').controller('WareHousePickupNTCtrl', ['$scope', '$filter', 'Config_Status', 'Warehouse',
 	function($scope, $filter, Config_Status, Warehouse) {
        ...
        $scope.frm                  = {type_process : 13, group : 2, domain: 'boxme.vn', 'location' : 1};

        $scope.list_data            = {};
        $scope.total_group          = {};

        $scope.__get_list_pipe_status(2,13);
        ...
        $scope.exportExcel = function(){
            ...
            var html =
                ...
                "<td style='border-style:none'></td>"+
                "<td colspan='3' style='font-size: 18px; border-style:none '><strong>Danh Sách Lấy Hàng Chậm Nội Thành</strong></td></tr>" +
                "<tr></tr>" +
            ...
            Warehouse.pickup_slow(1,$scope.frm, 'export').then(function (result) {
                if(!result.data.error){
                    ...
                    saveAs(blob, "danh_sach_lay_hang_cham_noi_thanh.xls");
                }
            }).finally(function() {
                $scope.waiting_export   = false;
            });
