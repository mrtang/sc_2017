...
angular.module('app').controller('WareHousePackedCtrl', ['$scope', 'Warehouse',
 	function($scope, Warehouse) {
        ...
        $scope.setPage = function(page){
            ...
            $scope.refresh('');
            Warehouse.package($scope.currentPage,$scope.frm).then(function (result) {
        ...
        $scope.setCountGroup    = function(){
            $scope.total_group  = [];
            Warehouse.package_count_group($scope.frm).then(function (result) {
                if(!result.data.error){
        ...
        $scope.exportExcel = function(){
            ...
            "<table width='100%' border='1'>" +
                ...
                "<td style='border-style:none'></td>"+
                "<td colspan='3' style='font-size: 18px; border-style:none '><strong>Danh Sách đóng gói</strong></td></tr>" +
                "<tr></tr>" +
                "<tr style='font-size: 14px; background: #6b94b3'>" +
                "<th rowspan='2'>STT</th>" +
                "<th rowspan='2'>TG Tạo</th>" +
                "<th rowspan='2'>TG nhập kho</th>" +
            ...
            Warehouse.package(1,$scope.frm, 'export').then(function (result) {
                if(!result.data.error){
                    angular.forEach(result.data.data, function(value, key) {
                        html+= "<tr>" +
                            "<td>"+ i++ +"</td>" +
                            "<td>"+ value.create +"</td>" +
                            "<td>"+ value.time_stock +"</td>" +
                            "<td>"+ value.time_packge +"</td>" +
                            ...
                            "<td>"+ ((value.__get_package != undefined && value.__get_package.size != undefined) ? value.__get_package.size : '') +"</td>" +
                            ...
                            "<td>"+ (($scope.package_status[value.__get_package.status] != undefined) ? $scope.package_status[value.__get_package.status]['name'] : '') +"</td>" +
                    ...
                    saveAs(blob, "danh_sach_package.xls");
                }
            }).finally(function() {
                $scope.waiting_export   = false;
            });
