...
angular.module('app').controller('WareHousePackageNTCtrl', ['$scope', '$filter', 'Config_Status', 'Warehouse',
 	function($scope, $filter, Config_Status, Warehouse) {
        ...
        $scope.frm                  = {type_process : 13, group : 1, domain: 'boxme.vn', 'location' : 1};

        $scope.list_data            = {};
        $scope.total_group          = {};

        $scope.__get_list_pipe_status(3,13);
        ...
        $scope.setPage = function(page){
            ...
            $scope.refresh('');
            Warehouse.package_slow($scope.currentPage,$scope.frm, '').then(function (result) {
        ...
        $scope.setCountGroup    = function(){
            $scope.total_group  = [];
            Warehouse.package_slow_count_group($scope.frm).then(function (result) {
                if(!result.data.error){
        ...
        $scope.exportExcel = function(){
            ...
            var html =
                ...
                "<td style='border-style:none'></td>"+
                "<td colspan='3' style='font-size: 18px; border-style:none '><strong>Danh Sách Đóng Gói Chậm Nội Thành</strong></td></tr>" +
                "<tr></tr>" +
            ...
            Warehouse.package_slow(1,$scope.frm, 'export').then(function (result) {
                if(!result.data.error){
                    ...
                    saveAs(blob, "danh_sach_dong_goi_cham_noi_thanh.xls");
                }
            }).finally(function() {
                $scope.waiting_export   = false;
            });
