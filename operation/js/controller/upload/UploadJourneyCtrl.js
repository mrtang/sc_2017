...
angular.module('app').controller('UploadJourneyCtrl', ['$scope', '$state', '$stateParams', '$rootScope', 'FileUploader', 'toaster', 'Api_Path', 'Upload', 'Config_Status',
 	function($scope, $state, $stateParams, $rootScope,FileUploader, toaster, Api_Path, Upload, Config_Status) {
    /*
        Config
    */
        $scope.currentPage          = 1;
        ...
        var uploader = $scope.uploader = new FileUploader({
            url                 : Api_Path.Upload+'upload?type=journey',
        ...
        uploader.onBeforeUploadItem = function(item) {
            item.url = Api_Path.Upload+'upload?type=journey&courier_id='+$scope.courier_id;
        };
        ...
        uploader.onSuccessItem = function(item, result, status, headers){
            ...
            if(!result.error){
                ...
                $state.go('delivery.upload.upload_journey',{id:result.id});
            }
            else{
     ...
     */
        ...
        $scope.Verify = function(){
            Upload.Journey($stateParams.id).then(function (result) {
        ...
        $scope.exportExcel = function(){
            return Upload.ListUpload($stateParams.id, $scope.currentPage, $scope.frm, 'JOURNEY');
        }
