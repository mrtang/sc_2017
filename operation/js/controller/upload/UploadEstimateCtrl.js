...
angular.module('app').controller('UploadEstimateCtrl', ['$scope', '$state', '$filter', '$stateParams', '$rootScope', 'FileUploader', 'toaster', 'Api_Path', 'Upload', 'Config_Status',
    ...
    */
        ...
        var uploader = $scope.uploader = new FileUploader({
            url                 : Api_Path.Upload+'upload?type=estimate',
        ...
        uploader.onBeforeUploadItem = function(item) {
            item.url = Api_Path.Upload+'upload?type=estimate&courier_id='+$scope.courier_id+'&service_id='+$scope.service_id;
        };
        ...
        uploader.onSuccessItem = function(item, result, status, headers){
            ...
            if(!result.error){
                ...
                $state.go('delivery.upload.upload_estimate',{id:result.id});
            }
            else{
     ...
     */
        ...
        $scope.Verify = function(){
            Upload.Estimate($stateParams.id).then(function (result) {
        ...
        $scope.export_excel = function(){
            ...
            var html =
                ...
                "<th rowspan='2'>Thời gian cam kết</th>" +
                ...
                "<tbody>";
            var i = 1;
            Upload.ListUpload1($stateParams.id, $scope.currentPage,$scope.frm, 'ESTIMATE').then(function (result) {
                if(!result.data.error){
                    angular.forEach(result.data.data, function(value) {
                        html+= "<tr>" +
                            ...
                            "<td>"+ value.to_city +"</td>" +
                            "<td>"+  (($scope.city[1*value.sc_from_city])           ? $scope.city[1*value.sc_from_city]                 : '') +"</td>" +
                            "<td>"+  (($scope.city[1*value.sc_to_city])             ? $scope.city[1*value.sc_to_city]                 : '') +"</td>" +
                    ...
                    saveAs(blob, "danh_sach_cap_nhat_thoi_gian_cam_ket.xls");
                }
            }).finally(function() {
                $scope.waiting_export   = false;
            });
