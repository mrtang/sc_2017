...
angular.module('app').controller('InTransitCtrl', ['$scope', '$rootScope', '$filter', 'Order', 'Config_Status', 'Base',
 	function($scope, $rootScope, $filter, Order, Config_Status, Base) {
        ...
        $scope.frm                  = {group : 71, global: 0, list_status : ''};
        ...
        if($scope.group_order_status[71] == undefined){
            $scope.group_order_status[71]   = Config_Status.group_status[71];
        }
        ...
        Base.PipeStatus(71, 1).then(function (result) {
        ...
        $scope.refresh = function(cmd){
            if($scope.frm.list_status == undefined || $scope.frm.list_status.length  == 0){
                $scope.frm.list_status  = $scope.group_order_status[71].toString();
            }
        ...
        $scope.exportExcel = function(){
            ...
            var html =
                ...
                "<td style='border-style:none'></td>"+
                "<td colspan='3' style='font-size: 18px; border-style:none '><strong>Danh sách đang vận chuyển</strong></td></tr>" +
                "<tr></tr>" +
            ...
            Order.GetOrder($scope.currentPage,data,'export').then(function (result) {
                if(!result.data.error){
                    ...
                    saveAs(blob, "Danh_sach_dang_van_chuyen.xls");
                }
            }).finally(function() {
