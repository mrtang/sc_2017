...
angular.module('app').controller('DeliveredCtrl', ['$scope', '$rootScope', 'Order', 'Config_Status', 'Base',
 	function($scope, $rootScope, Order, Config_Status, Base) {
        ...
        $scope.list_post_office = [];
        if($scope.group_order_status[30] == undefined){
            $scope.group_order_status[30]   = Config_Status.group_status[30];
        }
        ...
        Base.PipeStatus(30, 1).then(function (result) {
        ...
        $scope.setPage = function(page){
            ...
            $scope.refresh('');
            Order.ListOrder($scope.currentPage,$scope.frm, $scope.group_order_status[30].toString(), '').then(function (result) {
                if(!result.data.error){
        ...
        $scope.setCountGroup    = function(){
            $scope.total_all    = 0;
            $scope.total_group  = [];
            Order.CountGroup($scope.frm, $scope.group_order_status[30].toString()).then(function (result) {
                if(!result.data.error){
        ...
        $scope.exportExcel = function(){
            ...
            return Order.ListOrder(1,$scope.frm,$scope.group_order_status[30].toString(),'export');
        }
    }
]);
