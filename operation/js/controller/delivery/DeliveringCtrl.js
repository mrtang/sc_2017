...
angular.module('app').controller('DeliveringCtrl', ['$scope', '$rootScope', 'Order', 'Config_Status', 'Base',
 	function($scope, $rootScope, Order, Config_Status, Base) {
        ...
        if($scope.group_order_status[28] == undefined){
            $scope.group_order_status[28]   = Config_Status.group_status[28];
        }
        ...
        $scope.refresh = function(cmd){
            if($scope.tab_status == undefined || $scope.tab_status.length  == 0){
                $scope.tab_status  = $scope.group_order_status[28].toString();
            }
