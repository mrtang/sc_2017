...
angular.module('app').controller('DProblemCtrl', ['$scope', '$rootScope', '$filter', 'Order', 'Config_Status', 'Base',
 	function($scope, $rootScope, $filter, Order, Config_Status, Base) {
        ...
        $scope.frm                  = {group : 29, global: 0, list_status : ''};
        ...
        if($scope.group_order_status[29] == undefined){
            $scope.group_order_status[29]   = Config_Status.group_status[29];
        }
        ...
        Base.PipeStatus(29, 1).then(function (result) {
        ...
        $scope.refresh = function(cmd){
            if($scope.frm.list_status == undefined || $scope.frm.list_status.length  == 0){
                $scope.frm.list_status  = $scope.group_order_status[29].toString();
            }
        ...
        $scope.exportExcel = function(){
            ...
            var html =
                ...
                "<td style='border-style:none'></td>"+
                "<td colspan='3' style='font-size: 18px; border-style:none '><strong>Danh sách phát thất bại</strong></td></tr>" +
                "<tr></tr>" +
                "<tr style='font-size: 14px; background: #6b94b3'>" +
                "<th rowspan='2'>STT</th>" +
                "<th rowspan='2'>TG Duyệt</th>" +
                "<th rowspan='2'>TG Lấy Hàng</th>" +
                "<th rowspan='2'>TG PTB-CXL</th>" +
                "<th rowspan='2'>TG PTB-LDK</th>" +
            ...
            Order.GetOrder($scope.currentPage,data,'export').then(function (result) {
                if(!result.data.error){
                    ...
                    angular.forEach(result.data.data, function(value) {
                        ...
                        if(value.order_status != undefined){
                            angular.forEach(value.order_status, function(v) {
                                note += v.note+',';
                                if(v.status*1 == 77){
                                    time_ptb    = v.time_create;
                                }
                                if(v.status*1 == 57){
                                    time_ldk    = v.time_create;
                                }
                            });
                        }
                        ...
                        html+= "<tr>" +
                            ...
                            "<td>"+  (time_ptb  > 0 ? $filter('date')(time_ptb*1000, "dd/MM/yyyy  HH:mm:ss") : '') +"</td>" +
                            "<td>"+  (time_ldk  > 0 ? $filter('date')(time_ldk*1000, "dd/MM/yyyy  HH:mm:ss") : '') +"</td>" +
                    ...
                    saveAs(blob, "Danh_sach_phat_that_bai.xls");
                }
            }).finally(function() {
