...
angular.module('app').controller('StatusProblemCtrl', ['$scope', '$rootScope', 'Order', 'Config_Status', 'Base',
 	function($scope, $rootScope, Order, Config_Status, Base) {
        ...
        if($scope.group_order_status[35] == undefined){
            $scope.group_order_status[35]   = Config_Status.group_status[35];
        }
        ...
        Base.PipeStatus(35, 1).then(function (result) {
        ...
        $scope.refresh = function(cmd){
            if($scope.tab_status == undefined || $scope.tab_status.length  == 0){
                $scope.tab_status  = $scope.group_order_status[35].toString();
            }
