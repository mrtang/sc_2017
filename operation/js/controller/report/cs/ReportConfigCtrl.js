...
angular.module('app').controller('ReportConfigCtrl', ['$scope', '$rootScope', '$filter', 'Base', 'Report',
 	function($scope, $rootScope, $filter, Base, Report) {
        ...
        $scope.__color_by_percent   = function(data){
            if(data != undefined && data.percent != undefined && data.percent_target != undefined){
                if(data.percent < data.percent_target){
                    return 'text-warning';
                }else{
                    return 'text-success';
                }
        ...
        $scope.$watch('frm.country_id', function(newValue, oldValue) {
            if(newValue != undefined){
                Base.kpi_group_config({group: 6, country_id: newValue, active : 1})
