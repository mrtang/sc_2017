...
angular.module('app').controller('ProblemCtrl_', ['$scope', '$http', '$state', '$window', '$modal', '$rootScope', 'toaster', 'Order', 'Config_Status', 'Base',
 	function($scope, $http, $state, $window, $modal, $rootScope, toaster, Order, Config_Status, Base) {
        ...
        $scope.frm                  = {group : 26, global: 0, vip : 0, location : 1, to_city : 18, from_city : 18, slow: 0};
        ...
        if($scope.group_order_status[26] == undefined){
            $scope.group_order_status[26]   = Config_Status.group_status[26];
        }
        ...
        Base.PipeStatus(26, 1).then(function (result) {
        ...
        $scope.refresh = function(cmd){
            ...
            if($scope.time.accept_end != undefined && $scope.time.accept_end != ''){
                $scope.frm.accept_end      = +Date.parse($scope.time.accept_end)/1000;
            }else{
            ...
            if($scope.check_box_status != undefined && $scope.check_box_status.length > 0){
                ...
                $scope.tab_status       =    $scope.group_order_status[26].toString();
            }
