...
angular.module('app').controller('AcceptCtrl', ['$scope', '$rootScope', 'Order', 'Config_Status',
 	function($scope, $rootScope, Order, Config_Status) {
        ...
        if($scope.group_order_status[24] == undefined){
            $scope.group_order_status[24]   = Config_Status.group_status[24];
        }
        ...
        $scope.setPage = function(page){
            ...
            $scope.refresh('');
            Order.ListOrder($scope.currentPage,$scope.frm, $scope.group_order_status[24].toString(), '').then(function (result) {
        ...
        $scope.setCountGroup    = function(){
            $scope.total_all    = 0;
            $scope.total_group  = [];
            Order.CountGroup($scope.frm, $scope.group_order_status[24].toString()).then(function (result) {
                if(!result.data.error){
        ...
        $scope.exportExcel = function(){
            ...
            return Order.ListOrder(1,$scope.frm,$scope.group_order_status[24].toString(),'export');
        }
