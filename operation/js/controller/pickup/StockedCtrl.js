...
angular.module('app').controller('StockedCtrl', ['$scope', '$rootScope', 'Order', 'Config_Status',
 	function($scope, $rootScope, Order, Config_Status) {
        ...
        if($scope.group_order_status[27] == undefined){
            $scope.group_order_status[27]   = Config_Status.group_status[27];
        }
        ...
        $scope.setPage = function(page){
            ...
            $scope.refresh('');
            Order.ListOrder($scope.currentPage,$scope.frm, $scope.group_order_status[27].toString(), '').then(function (result) {
        ...
        $scope.setCountGroup    = function(){
            $scope.total_all    = 0;
            $scope.total_group  = [];
            Order.CountGroup($scope.frm, $scope.group_order_status[27].toString()).then(function (result) {
                if(!result.data.error){
        ...
        $scope.exportExcel = function(){
            ...
            return Order.ListOrder(1,$scope.frm,$scope.group_order_status[27].toString(),'export');
        }
    }
]);
