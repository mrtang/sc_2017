...
angular.module('app').controller('ReportStoreCtrl', ['$scope', '$rootScope', '$filter', 'Order', 'Config_Status',
 	function($scope, $rootScope, $filter, Order, Config_Status) {
        ...
        $scope.setPage = function(page){
            ...
            $scope.refresh('');
            Order.ListReportStore($scope.currentPage,$scope.frm, $scope.tab_status, '').then(function (result) {
        ...
        $scope.setCountGroup    = function(){
            ...
            $scope.total_group  = [];
            $scope.list_reponse = {};
            Order.CountReportStock($scope.frm, '','status').then(function (result) {
                if(!result.data.error){
        ...
        $scope.exportExcel = function(){
            ...
            Order.ListReportStore($scope.currentPage,$scope.frm, $scope.tab_status, 'export').then(function (result) {
                if(!result.data.error){
                    ...
                    saveAs(blob, "Yeu_cau_luu_kho.xls");
                }
            }).finally(function() {
                $scope.waiting_export   = false;
            });
