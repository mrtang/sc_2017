...
angular.module('app').controller('RecoverCtrl', ['$scope', '$http', '$state', '$stateParams', '$rootScope', 'toaster', 'FileUploader', 'Recover', 'Api_Path', 'Config_Status',
 	function($scope, $http, $state, $stateParams, $rootScope, toaster, FileUploader, Recover, Api_Path, Config_Status) {
        /*
         Config
         */
        $scope.currentPage          = 1;
        ...
        $scope.totalItems           = 0;
        $scope.NewTotal             = 0;
        $scope.frm                  = {id : $stateParams.id, tab : 'ALL', type : 'recover'};
        $scope.data_sum             = 0;
        $scope.waiting              = true;
        ...
        var uploader = $scope.uploader = new FileUploader({
            url                 : Api_Path.Acc+'cash-out/upload-recover',
            alias               : 'AccFile',
        ...
        uploader.onSuccessItem = function(item, result, status, headers){
            if(!result.error){
                ...
                $state.go('app.recover',{id:result.id});
            }
            else{
        ...
        $scope.load = function(page){
            ...
            Recover.load_excel($scope.currentPage, $scope.frm, '').then(function (result) {
        ...
        $scope.Verify = function(){
            Recover.verify($stateParams.id).then(function (result) {
                if($scope.status_error.indexOf(result.status) != -1){
                    ...
                    if(!result.data.error){
                        ...
                        toaster.pop('warning', 'Thông báo', $scope.list_status[result.data.message].text);
                    }
