...
angular.module('app').controller('VerifyCtrl', ['$scope', '$state', '$stateParams', '$rootScope', 'toaster', 'FileUploader', 'CourierVerify', 'MerchantVerify','Api_Path', 'Config_Status',
 	function($scope, $state, $stateParams, $rootScope, toaster, FileUploader, CourierVerify, MerchantVerify, Api_Path, Config_Status) {
        /*
         Config
         */
        $scope.currentPage          = 1;
        ...
        $scope.totalItems           = 0;
        $scope.NewTotal             = 0;
        $scope.frm                  = {id : $stateParams.id, tab : 'ALL', type : 'money_collect'};
        $scope.data_sum             = 0;
        $scope.waiting              = true;
        ...
        var uploader = $scope.uploader = new FileUploader({
            url                 : Api_Path.Acc+'verify/upload',
        ...
        uploader.onSuccessItem = function(item, result, status, headers){
            if(!result.error){
                ...
                $state.go('app.payment_verify',{id:result.id});
            }
            else{
        ...
        $scope.Verify = function(){
            MerchantVerify.Verify($stateParams.id).then(function (result) {
