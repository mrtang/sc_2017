...
angular.module('app').controller('CashOutCtrl', ['$scope', '$state', '$stateParams', '$rootScope', 'toaster', 'FileUploader', 'CashOut', 'Api_Path', 'Config_Status',
 	function($scope, $state, $stateParams, $rootScope, toaster, FileUploader, CashOut, Api_Path, Config_Status) {
        /*
         Config
         */
        $scope.currentPage          = 1;
        ...
        $scope.totalItems           = 0;
        $scope.NewTotal             = 0;
        $scope.frm                  = {id : $stateParams.id, tab : 'ALL', type : 'cash_out'};
        $scope.data_sum             = 0;
        $scope.waiting              = true;
        ...
        var uploader = $scope.uploader = new FileUploader({
            url                 : Api_Path.Acc+'cash-out/upload-https',
        ...
        uploader.onSuccessItem = function(item, result, status, headers){
            if(!result.error){
                ...
                $state.go('app.cash_out',{id:result.id});
            }
            else{
        ...
        $scope.load = function(page){
            ...
            CashOut.load_excel($scope.currentPage, $scope.frm, '').then(function (result) {
        ...
        $scope.Verify = function(){
            CashOut.verify($stateParams.id).then(function (result) {
