...
.factory('sessionService', ['$http', '$localStorage', function($http, $localStorage){
    return{
        ...
        destroy:function(){
            $localStorage.$reset();
            return $http.get(ApiPath+'api/v1/app/checkout');
        }
    };
}])
.factory('loginService',function($http, $rootScope, $location, sessionService, $localStorage, $window){
    return{
        login:function(data,scope,state){
            $http({
                url: ApiPath+'api/v1/app/checkin',
                method: "POST",
                data: data,
                dataType: 'json',
                ...
                if(result.code == 'success'){
                    if((result['data']['privilege'] != 2) && (result['data']['group_privilege']['PRIVILEGE_ACCOUNTING'] == undefined || result['data']['group_privilege']['PRIVILEGE_ACCOUNTING']['view'] != 1)){
...
.run(
  [ '$rootScope', '$state', '$http', '$stateParams', '$location', 'loginService', 'sessionService', 'Privilege',
  function ($rootScope, $state, $http, $stateParams, $location, loginService, sessionService, Privilege) {
        $rootScope.$state = $state;
        $rootScope.$stateParams = $stateParams;
        var routespermission = ['/access/signin','/access/signup'];
        var router_privilege = Privilege.router;
      ...
      $rootScope.$on('$stateChangeStart', function(evt,toState){
            ...
            if (!toState.name.match(/^access/g))
            {
                ...
                }else if(!toState.name.match(/^provider/g))
                {
