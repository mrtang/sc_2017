'use strict';
//Courier Fee
angular.module('app').controller('CourierFeeVasCtrl', ['$scope','$filter','$http','$state','$window','toaster','bootbox', 
    ...
    $http.get(ApiPath+'courier-vas').success(function(result){
        if(result.data){
            angular.forEach(result.data, function(value, key) {
              $scope.list_courier_vas[value.id] = value.name;
            });
        }
    });
    ...
    $scope.GetCourierFee = function(){
        if($scope.search_courier > 0){
        	$http({
                url: ApiPath+'courier-vas-fee?page='+$scope.currentPage+'&courier_id='+$scope.search_courier,
                method: "GET",
                dataType: 'json',
    ...
    $scope.change = function(id,data,field) {
        ...
        myData[field] = data;
        $http({
            url: ApiPath+'courier-vas-fee/edit/'+id,
            method: "POST",
            data:myData,
            dataType: 'json'
        }).success(function (result, status, headers, config) {
    ...
    $scope.del = function(id,item){
        bootbox.confirm("Bạn chắc chắn muốn xóa ?", function (result) {
            if(result){
            	$http({
                    url: ApiPath+'courier-vas-fee/destroy/'+id,
                    method: "get",
                    dataType: 'json',
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                }).success(function (result, status, headers, config) {
